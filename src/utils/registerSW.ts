...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/pdf-to-image/sw.js, scope=/pdf-to-image/）。
    // scope がツールの全 locale ページ（/pdf-to-image/, /pdf-to-image/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/pdf-to-image/sw.js', {
      scope: '/pdf-to-image/',
    });
